...
let box_fixer  = document.getElementById('box_fixer')
...
function start() {
  input.disabled = true; 
  box_fixer.classList.add("hidden");
...
function cashout(){
 ...
 cashout_event.style.display = ' none '
  ...
  if(tabdil <= point){
      box_fixer.classList.remove("hidden");
...
function boxmaker(){
  ...
  const text = document.createTextNode(point_adad_for_box + ' ' + 'X');
  box.appendChild(text);
  if(colorpicker === 1){
    box.classList.add('tytel_x_shower_inbox_inner')
  }
  if(colorpicker === 2){
    box.classList.add('tytel_x_shower_inbox_inner_loss')
  }

  box.classList.add('flex')
  box.setAttribute('id', 'A'+val);
  tt.appendChild(box);
  val+=1;

  if(val === 9 && responsivenumber === 0  ){
    document.getElementById("A"+1).remove();
    document.getElementById("A"+2).remove();
    document.getElementById("A"+3).remove();
    document.getElementById("A"+4).remove();
    document.getElementById("A"+5).remove();
    document.getElementById("A"+6).remove();
    document.getElementById("A"+7).remove();
    document.getElementById("A"+8).remove();
    val = 1
    boxmaker()
  }
  if(val === 6 && responsivenumber === 1  ){
    document.getElementById("A"+1).remove();
    document.getElementById("A"+2).remove();
    document.getElementById("A"+3).remove();
    document.getElementById("A"+4).remove();
